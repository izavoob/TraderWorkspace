...
ipcMain.handle('getSTERAssessmentsByPostSessionId', async (event, postSessionId) => {
  await ensureDatabaseInitialized();
  return new Promise((resolve, reject) => {
    console.log('Getting STER assessments for post session', postSessionId);
    
    sterDB.getAssessmentsByPostSessionId(postSessionId)
      .then(assessments => {
        console.log(`Found ${assessments.length} STER assessments for post session`);
        resolve(assessments);
      })
      .catch(err => {
        console.error('Error getting STER assessments for post session:', err);
        reject(err);
      });
  });
});
